...
class TensorSInput:
    def __init__(self, batch_size, length):
        self.words = Variable(torch.LongTensor(batch_size, length).zero_(), requires_grad=False)
        self.heads = [[[] for idx in range(length)] for idy in range(batch_size)]
        self.childs = [[[] for idx in range(length)] for idy in range(batch_size)]
        self.masks = Variable(torch.Tensor(batch_size, length).zero_(), requires_grad=False)

    def to_cuda(self, device):
        self.words = self.words.cuda(device)
        self.masks = self.masks.cuda(device)

    @property
    def inputs(self):
        return (self.words, self.heads, self.childs, self.masks)
