...
class BiLSTMModel(nn.Module):
    ...
    def forward(self, inputs):
        ...
        src_hiddens, src_state = self.lstm(src_embed, src_masks, None)
        ...
        tgt_hiddens, tgt_state = self.lstm(tgt_embed, tgt_masks, None)
        ...
        if self.use_cosine:
            sim = F.cosine_similarity(src_represents, tgt_represents, dim=1).reshape(batch_size, 1)
            other = torch.ones(sim.shape).to(sim.device)
            other = other - sim
            outputs = torch.cat((sim, other), dim=1)
            pass
        else:
            diff_hiddens = src_represents - tgt_represents
            hiddens = diff_hiddens * diff_hiddens
            outputs = self.proj(hiddens)
        return outputs, src_represents, tgt_represents
    ...
    def method(self, inputs):
        ...
        hiddens, state = self.lstm(embed, masks, None)
    ...
    def predict(self, src_represents, tgt_represents):
        n, l = src_represents.shape
        if self.use_cosine:
            sim = F.cosine_similarity(src_represents, tgt_represents, dim=1).reshape(n, 1)
            other = torch.ones(sim.shape).to(sim.device)
            other = other - sim
            outputs = torch.cat((sim, other), dim=1)
        else:
