...
#     word is represented as tuple of symbols (symbols being variable-length strings)
#     """
#     tokens = re.sub(r'\$\$',' ',word).split()
#     pairs = set()
#     prev_token = tokens[0]
#     for cur_token in tokens[1:]:
#         pairs.add((prev_token, cur_token))
...
def encode(orig, bpe_codes, bpe_codes_reverse, vocab, separator, version, cache, glossaries=None):
    ...
    if version == (0, 1):
        ...
        tokens = re.sub(r'\$\$', ' ', orig).split()
        # word = tuple(orig[:-1]) + (orig[-1] + '</w>',)
        word = tuple(tokens[:-1]) + (tokens[-1] + '</w>',)
    else:
...
if __name__ == '__main__':
    ...
    bpe = BPE(args.codes, -1, '@@', None, None)

    i = 0
    for line in args.input:
        i+=1
        # print(line)
        after_process = bpe.process_line(line)
        # print(after_process)
        if i % 2142 == 0:
            pass
        if '$$' in after_process:
            exit(-1)
        args.output.write(after_process)
